...
Value xListTransactions(int64 nCount, int64 nMinDepth, bool fGenerated)
{
    vector<txnitem> tv;
    CRITICAL_BLOCK(cs_mapWallet)
    {
...
Value xlisttransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            "xlisttransactions [count=10] [minconf=1] [includegenerated=true]\n"
    ...
    return xListTransactions(nCount, nMinDepth, fGenerated);
}
...
pair<string, rpcfn_type> pCallTable[] =
{
    ...
    make_pair("xlisttransactions",     &xlisttransactions),
...
int CommandLineRPC(int argc, char *argv[])
{
    ...
    try
    {
        ...
        if (strMethod == "xlisttransactions"      && n > 0) ConvertTo<boost::int64_t>(params[0]);
        if (strMethod == "xlisttransactions"      && n > 1) ConvertTo<boost::int64_t>(params[1]);
        if (strMethod == "xlisttransactions"      && n > 2) ConvertTo<bool>(params[2]);
